...
public class UserDao {
	 ...
	 * @return
	 */
	public List<UserForTransfer> getUserListByClasses(int gid, int mid, int cid) {
		// UserForTransfer 的String 类型 avatar 中 存Base64编码的图片
		List<UserForTransfer> backUserForTransfers = new ArrayList<UserForTransfer>();
		...
		try {
			...
			while (rs.next()) {
				...
				UserForTransfer transfer = new UserForTransfer();

				transfer.setUid(rs.getInt("Uid"));
				transfer.setUname(rs.getString("Uname"));

				String strUavatar = rs.getString("Uavatar");
				java.io.File file = new java.io.File(strUavatar);
				String base64avatar = Base64Util.readFileInBASE64(file);

				transfer.setUavatar(base64avatar);
